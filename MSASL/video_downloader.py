...
def download_yt_videos(indexfile, saveto='raw_videos_trainset'):
    # content = json.load(open(indexfile))[1700:]
    content = json.load(open(indexfile))
    appeared = []
    ...
    for entry in content:
        video_url = entry['url']
        if video_url in appeared:
            continue
        appeared.append(video_url)

        if 'youtube' not in video_url and 'youtu.be' not in video_url:
            continue

        if os.path.exists(os.path.join("cropped_videos_train", video_url[-11:] + '1.mp4')) or \
            os.path.exists(os.path.join(saveto, video_url[-11:] + '.mp4')) or \
            os.path.exists(os.path.join(saveto, video_url[-11:] + '.mkv')) or \
            os.path.exists(os.path.join(saveto, video_url[-11:] + '.webm')):
            logging.info('YouTube videos {} already exists.'.format(video_url))
            continue
        else:
            cmd = "yt-dlp \"{}\" -o \"{}%(id)s.%(ext)s\""
...
if __name__ == '__main__':
    ...
    download_yt_videos('MSASL_train.json')